...
- `knowledge_search`：`{ query?, tags?, author?, sourceType?, credibility?, limit?=20, detail?='summary' }`，大小写不敏感子串匹配（title/summary/coreClaims/tags）+ 过滤；有关键词时按字段命中相关度排序（tags > title > coreClaims > summary/author，同分按 updatedAt 倒序），无关键词按 updatedAt 倒序；`detail="full"` 附核心论点/事实核查/经验/边界全文（2026-09-02 演进，见 Agent Note 2026-09-02-journal-agents-knowledge-recall）；
