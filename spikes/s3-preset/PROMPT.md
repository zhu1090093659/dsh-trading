...
【你的任务 S3】验证「第三方 bundle 给 agent-presets 配额外 root + 插件自安装 preset 目录被发现/挂载」。packages/preset/agent-presets/README.zh.md 必读；patch 语义整行替换。
工作目录：/Users/zcl/code/dsh-trading/spikes/s3-preset/；专用 profile 名：spike-s3。
核心步骤：精读 agent-presets README 与 src/discovery.ts、src/mount.ts（roots 格式、发现时机、新目录感知、broken 语义、preset.yml 与 agent.cordis.yml 格式，参照官方 presets/）；做 spike-preset-pkg（bundle）：patch 给 agent-presets 行配 roots（整行替换语义，default 等必填键 restate，查 config-catalog）；插件启动时把 preset 幂等写入 root；装进 spike-s3 验证 roster 出现且会话可挂载（选最可靠观察面并说明理由）；加测不重启新增 preset 目录的感知、卸载后 broken 行为。
通过标准：preset 被发现且会话可挂载；自安装路径实测结论明确（可行/需重启/不可行）。REPORT.md 给出准确文件格式与 patch 写法。