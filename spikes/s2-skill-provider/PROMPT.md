...
【你的任务 S2】验证「第三方包通过代码注册 skill provider，skill 随包分发后对模型可见可加载」。官方证据：packages/skill/skill-badge/src/index.ts。
工作目录：/Users/zcl/code/dsh-trading/spikes/s2-skill-provider/；专用 profile 名：spike-s2。
注意：此前一轮被中断的执行留下了 spike-skill-pkg（含 index.js/cordis.patch.yml/assets）与 headless-overlay.yml。先审计再利用。
核心步骤：精读 skill-badge/src/index.ts、packages/skill/skill/README.zh.md、docs/subsystems/skills.zh.md、tool-skill；完善 spike-skill-pkg 注册 SkillProvider（skill 名带 spike 前缀）；装进 spike-s2；验证 skill 出现在会话目录且模型可经 skill 工具加载（headless 1 次短 prompt 或目录持久化文件直证，说明证据充分性）；加测卸载后消失。
通过标准：随包 skill 对模型可见可加载。REPORT.md 记录 provider 确切契约（文件:行号）、inject 要求、热刷新行为。