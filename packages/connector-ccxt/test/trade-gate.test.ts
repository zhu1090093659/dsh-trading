...
import { CcxtTradeService, type Config } from '../src/index.js'
...
function makeService(config: Config, routes: Array<{ match: string; body: unknown }> = []) {
  ...
  const trade = new CcxtTradeService(new CordisContext() as never, { exchange: 'binance', fetchImpl: impl, config })
  return { trade, urls }
}

const LIVE_REQ = { symbol: 'BTCUSDT', side: 'buy' as const, type: 'market' as const, quantity: 1, dryRun: false }

describe('CcxtTradeService 服务缝闸门（P0，绕过工具层直调）', () => {
