/**
 * CN（A 股）工具箱插件（dsh-trading cn+hk 双市场切片，模板 = kit-us）。
 *
 * 一件事：skill provider——cn-risk-checklist 随包分发（S2 形态；rank 用
 * BUNDLED_SKILL_RANK=600，用户目录 100-500 天然覆盖之；skill 名市场前缀命名空间）。
 *
 * 插件本体不被 host 面挂载（架构修订，与 kit-crypto/kit-us 同款）：本行在 cn-trader
 * preset 的 agent.cordis.yml 内，preset 级会话隔离——skill 注册表按 scope 分层。
 * preset 自安装不在本插件（在 @dsh-trading/cn bundle 的常驻安装器行）。
 *
