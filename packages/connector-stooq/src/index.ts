 ...
 *
 * 下单三段闸门语义照抄 crypto 切片（README 铁律 #3 / S4 修订）：工具名 `us_place_order`
 * 落在 base 闸门模式 `/^(?:crypto|us|cn|hk)_(?:place|cancel)_order$/`；审批不在工具内做。
 * Stooq 本身无交易 API——live 路径恒为 TRADING_NOT_IMPLEMENTED（券商 API 是后续任务）。
 *
 ...
 */
export const name = 'dsh-trading-us-connector-stooq'
...
export interface SubscribeTickerOptions {
  /** 轮询间隔（ms）。subscribeTicker 以日 K 轮询实现（Stooq 无推送面）。 */
...
export class StooqMarketDataService extends Service implements MarketDataService {
  // TS 编译期 private 而非 ECMAScript #（cordis realm 代理按类身份炸，README 定稿 5）。
  private readonly client: StooqRestClient

  constructor(ctx: Context, options: StooqRestOptions = {}, name: string = TRADING_US_MARKET_DATA_KEY) {
    super(ctx, name)
    this.client = new StooqRestClient(options)
  }
  ...
  getKlines(symbol: string, interval: Interval, limit?: number): Promise<Kline[]> {
    return this.client.getHistorical(symbol, interval).then((all) =>
...
export interface PlaceOrderArgs {
  /** 股票代码，如 `AAPL` 或 `aapl.us`（执行前归一化为 Stooq 小写形态）。 */
...
/**
 * 闸门判定结果（三条路径与裁决顺序与 crypto 同构，见 connector-binance evaluateOrderGate）。
 */
...
function validatePlaceOrderArgs(args: PlaceOrderArgs): void {
  // 符号形态在这里只做宽松校验（非空字符串）；规范化/严格校验在服务层（normalizeStooqSymbol）。
  if (typeof args.symbol !== 'string' || !args.symbol.trim()) {
    throw new Error('us_place_order: invalid symbol — expected a US ticker like AAPL or aapl.us')
  }
  try {
    normalizeStooqSymbol(args.symbol)
  } catch (cause) {
...
/** DRY-RUN 回执：模拟成交 + Stooq 最新收盘参照（参照取不到不阻断模拟本身）。 */
...
export async function buildDryRunReceipt(
  ...
  const symbol = normalizeStooqSymbol(args.symbol)
  ...
  return JSON.stringify({
    ...
    dryRun: true,
    note:
      'DRY-RUN — simulated fill; no order was sent to any broker. Stooq is a data source only (no trading API); '
      + 'the reference price is the latest daily close, which does not reflect pre/post-market moves.',
  ...
  return defineTool({
    ...
    parameters: {
      symbol: {
        type: 'string',
        required: true,
        description: 'US stock ticker, e.g. AAPL or aapl.us',
      },
      side: {
        type: 'string',
        enum: ['BUY', 'SELL'],
    ...
    async execute(raw) {
      ...
      if (verdict.action === 'simulate') {
        // 闸门 ②：模拟成交回执（DRY-RUN 标记 + 收盘价参照）。
        return buildDryRunReceipt(args, deps.marketData)
      }
      // 闸门 ③：实盘执行未实现（Stooq 无交易 API；券商下单是后续任务）。
      throw new TradingServiceError(
        'TRADING_NOT_IMPLEMENTED',
        'us_place_order: live order execution is not implemented in this slice — Stooq provides market data only; '
...
export function apply(ctx: Context, config: Config): void {
  ...
  new StooqMarketDataService(ctx)
      ...
      defineTool({
        ...
        parameters: {
          symbol: {
            type: 'string',
            required: true,
            description: 'US stock ticker, e.g. AAPL or aapl.us (normalized internally)',
          },
        },
        output: {
          schema: { type: 'string' },
      ...
      defineTool({
        ...
        parameters: {
          ...
          interval: {
            ...
            description: 'Kline interval (Stooq-supported subset)',
            default: '1d',
          },
          limit: {
            type: 'integer',
            description: 'Number of most recent candles to return (1-1000; Stooq returns full history and the tool trims)',
            default: 100,
          },
        },
        output: {
