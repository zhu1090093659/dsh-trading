...
import { TushareTradeService, type Config } from '../src/index.js'
...
function makeService(config: Config, routes: Array<{ match: string; body: unknown }> = []) {
  ...
  const trade = new TushareTradeService(new CordisContext() as never, { token: 't', fetchImpl: impl, config })
  return { trade, urls }
}

const LIVE_REQ = { symbol: '600519.SH', side: 'buy' as const, type: 'market' as const, quantity: 1, dryRun: false }

describe('TushareTradeService 服务缝闸门（P0，绕过工具层直调）', () => {
  ...
  it('② dryRun 缺省直调 → 本地模拟回执（dryRun=true），不触网', async () => {
    const { trade, urls } = makeService(baseConfig())
    const order = await trade.placeOrder({ symbol: '600519.SH', side: 'buy', type: 'market', quantity: 1 })
