...
import { FmpMarketDataService, TRADING_US_MARKET_DATA_KEY, type Config } from './index.js'
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (registry === undefined) {
    new FmpMarketDataService(ctx, { apiKey })
    return
  }
  const inner = ctx.isolate(TRADING_US_MARKET_DATA_KEY)
  const service = new FmpMarketDataService(inner, { apiKey })
