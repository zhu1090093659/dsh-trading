...
export class FmpTradeService extends Service implements TradeService {
  ...
  constructor(
    ctx: Context,
    options: FmpRestOptions & { config: Config },
  ...
  async placeOrder(order: OrderRequest): Promise<Order> {
    ...
    if (!requestedDryRun && !this.config.liveTrading) {
      throw new TradingServiceError(
        ...
        `FMP TradeService.placeOrder rejected: the request asks for real execution (dryRun=${String(order.dryRun)}) `
  ...
  async cancelOrder(orderId: string): Promise<{ orderId: string; status: 'canceled' }> {
    ...
    if (!this.config.liveTrading || this.config.dryRun) {
      throw new TradingServiceError(
        'TRADING_LIVE_TRADING_DISABLED',
        'FMP TradeService.cancelOrder rejected at the service seam: cancel is a live action and requires liveTrading=true with dryRun=false.',
      )
    }
