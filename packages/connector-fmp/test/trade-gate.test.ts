...
import { FmpTradeService, type Config } from '../src/index.js'
...
function makeService(config: Config, routes: Array<{ match: string; body: unknown }> = []) {
  ...
  const trade = new FmpTradeService(new CordisContext() as never, { apiKey: 'k', fetchImpl: impl, config })
  return { trade, urls }
}

const LIVE_REQ = { symbol: 'AAPL', side: 'buy' as const, type: 'market' as const, quantity: 1, dryRun: false }

describe('FmpTradeService 服务缝闸门（P0，绕过工具层直调）', () => {
