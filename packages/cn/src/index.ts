/**
 * @dsh-trading/cn — cn 市场 bundle。包承载两块实质内容（实装形态抄 @dsh-trading/us）：
 *
 ...
 *      agent.cordis.yml（preset 级会话隔离，见 assets/preset/cn-trader/）。
 *   2. `dsh-trading-cn-installer` 插件（本模块）：cn-trader preset 幂等自安装（S3 机制）。
 ...
 * 本 bundle 的 preset 行挂 market=cn 实例（提供 tradingCnMarketData + cn_* 工具）。
 *
 * @module @dsh-trading/cn
 */
...
/**
 * Cordis 插件名 = patch 行 id（TEMPLATES §8）：`dsh-trading-cn-*` 市场命名空间，
...
// ── cn-trader preset 幂等自安装（S3 机制，抄 crypto/us-installer） ───────────────

export const PRESET_ID = 'cn-trader'
...
/**
 * 幂等自安装 cn-trader preset（S3 机制）：mkdir -p + 逐文件内容 diff 后写，内容一致则
...
export function apply(ctx: Context, config: Config): void {
  ...
  void installPreset({ presetRoot: config?.presetRoot }).then(
    (result) => logger(ctx).info(
      '[dsh-trading-cn-installer] self-install %s preset at %s wrote=[%s]',
      PRESET_ID,
      result.dir,
      result.wrote.join(',') || 'nothing — already current',
    ),
    (error: unknown) => logger(ctx).warn('[dsh-trading-cn-installer] cn-trader preset self-install failed: %s', error),
  )
}
