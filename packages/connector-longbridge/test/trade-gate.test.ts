...
import { LongbridgeTradeService, type Config } from '../src/index.js'
...
function baseConfig(overrides: Partial<Config> = {}): Config {
  return {
    ...
    dryRun: true,
    liveTrading: false,
  appKeyRef: 'LB_APP_KEY',
  appSecretRef: 'LB_APP_SECRET',
  accessTokenRef: 'LB_ACCESS_TOKEN',
    ...overrides,
  }
}
...
function makeService(config: Config, routes: Array<{ match: string; body: unknown }> = []) {
  ...
  const trade = new LongbridgeTradeService(new CordisContext() as never, { appKey: 'k', appSecret: 's', accessToken: 't', fetchImpl: impl, config })
  return { trade, urls }
}

const LIVE_REQ = { symbol: '00700.HK', side: 'buy' as const, type: 'market' as const, quantity: 1, dryRun: false }

describe('LongbridgeTradeService 服务缝闸门（P0，绕过工具层直调）', () => {
