...
import { IbkrTradeService, type Config } from '../src/index.js'
...
function makeService(config: Config, routes: Array<{ match: string; body: unknown }> = []) {
  ...
  const trade = new IbkrTradeService(new CordisContext() as never, { accountId: 'U123', fetchImpl: impl, config })
  return { trade, urls }
}

const LIVE_REQ = { symbol: 'AAPL', side: 'buy' as const, type: 'market' as const, quantity: 1, dryRun: false }

describe('IbkrTradeService 服务缝闸门（P0，绕过工具层直调）', () => {
