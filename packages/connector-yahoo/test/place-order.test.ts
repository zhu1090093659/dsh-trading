...
describe('us_place_order execute', () => {
  ...
  it('闸门 ②（缺省 dryRun）：DRY-RUN 模拟成交回执 + Yahoo regular-market 参照', async () => {
    ...
    expect(parsed.symbol).toBe('AAPL') // 入参 aapl/AAPL → 内部规范化为大写
    expect(parsed.reference.source).toBe('yahoo-regular-market')
    expect(parsed.reference.price).toBe(319.7)
    expect(getTicker).toHaveBeenCalledWith('AAPL')
  })
  ...
  it('闸门 ③：liveTrading=true + dryRun=false → TRADING_NOT_IMPLEMENTED（Yahoo 无交易 API）', async () => {
