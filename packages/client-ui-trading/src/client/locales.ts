...
export const zh: Record<MarketLocaleKey, string> = {
      ...
      'trade.holdings.import.guide': '请解析我接下来贴出的券商/交易所持仓截图，把其中的持仓用 holdings_stage 工具录入待确认区（只 stage 不 confirm，我会到资产抽屉确认入账）。要求：market 用 crypto/us/cn/hk 词汇；symbol 用连接器词汇（如 AAPL / 002714.SZ / BTCUSDT）；数字必须原样取自截图，不确定的字段缺省不编造；一张截图一个 account 名（我未说明时用截图里的券商名）。请等我贴图。',
...
export const en: Record<MarketLocaleKey, string> = {
      ...
      'trade.holdings.add': 'Add Manually',
