...
export const zh: Record<MarketLocaleKey, string> = {
      ...
      'quote.sendMenuSnapshot': '行情快照（含图表截图）',
      ...
      'compose.data.header': '【图表数据 · 与截图同一序列，可直接用于分析】',
      'compose.data.locator': '数据位置：market={market} · symbol={symbol} · interval={interval} · 共{count}根 · {range} · {tz}',
      'compose.data.refetch': '复核或取更长历史：Agent 可调用 {tool} 工具（symbol={symbol}，interval={interval}，limit={limit}）。',
      'compose.data.indicators': '指标复算：若已挂载 {tool} 工具，可同参数直接复算。',
      'compose.data.truncated': '以下内联最近 {inlined} 根（共 {count} 根），更早部分用上述工具复取。',
      'compose.data.full': '以下为全部 {count} 根的完整内联数据。',
      'compose.data.note': 'time = 各根K线开盘时间；指标空值 = warm-up 未就绪。',
...
export const en: Record<MarketLocaleKey, string> = {
      ...
      'quote.sendMenuSnapshot': 'Quote snapshot (with chart screenshot)',
      ...
      'compose.data.header': '[Chart data — same series as the screenshot; analyze it directly]',
      'compose.data.locator': 'Data locator: market={market} · symbol={symbol} · interval={interval} · {count} bars · {range} · {tz}',
      'compose.data.refetch': 'To verify or fetch longer history: call the {tool} tool (symbol={symbol}, interval={interval}, limit={limit}).',
      'compose.data.indicators': 'Indicator recompute: if the {tool} tool is mounted, recompute with the same params.',
      'compose.data.truncated': 'Inlined below: the most recent {inlined} of {count} bars — fetch earlier ones via the tool above.',
      'compose.data.full': 'All {count} bars are inlined below.',
      'compose.data.note': 'time = bar open time; empty indicator cells = warm-up.',
