...
- **The agent sees your screen.** One click on *Send to Agent* packages the symbol you're watching — live quote, current candle, the chart series' time range with a fetch locator (so the agent pulls the same routed data via its own tools and analyzes it in code), the computed readouts of the active indicators, and a screenshot of the chart itself — straight into the conversation. No copy-pasting numbers into a chatbot.
