...
- **Agent 看得到你的屏幕。** 点一下「发给 Agent」，你正在看的标的——实时报价、当前 K 线读数、图表序列的时间范围与取数位置（Agent 据此调工具取同源数据、写代码分析）、已开指标的当根读数、图表截图本身——全部自动打包进会话输入框。不再需要把数字复制粘贴给聊天机器人。
...
- **真实市场深度。** 实时盘口与买卖压力条、逐笔成交，以及衍生品驾驶舱——持仓量、资金费率与结算倒计时、多空人数比、主动买卖比——行情快照、资金面快照统一经报价头「发送给 Agent」按钮交给 Agent 分析（只填入会话输入框，不自动发送）。
