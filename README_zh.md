...
4、**一位大师，统领角色化团队。** `master`「大师」是全能型多 agent 团队主导者：拆解任务后，用带角色 persona 的子代理委派细分工作——标的分析交给研究员（`researcher_subagent`）、方案复核交给风控（`risk_reviewer_subagent`），交叉核对后整合成单一结论；跨市场速览与交易执行可亲自完成，同样默认 dry-run、实盘过闸。下设 `trader`「交易员」（交易计划与过闸执行）、`instrument-researcher`「标的分析研究员」（公告核验、基本面、估值与反方情景）、`risk-reviewer`「风险审查员」（敞口、压力情景与否决条件）。所有角色主动优先调用项目知识库、新闻公告和基本面工具。研究与风控保留只读行情/K线，不挂下单/撤单连接器；子代理继承会话工具面，角色 persona 约束职责边界——公共 host 工具仍可见，角色分工不是独立安全沙箱。

base 安装器按实际启用的市场生成四种角色，支持只安装部分市场。未修改的托管旧市场预设移到同级 `.legacy-backup` 备份 root；无管理戳、修改过或含额外文件的目录原样保留，不删除用户预设数据。
