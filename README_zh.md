...
4、**一个交易员，三种不同职责。** `trader`「交易员」整合已安装市场，负责交易计划与过闸执行；`instrument-researcher`「标的分析研究员」负责公告核验、基本面、估值与反方情景；`risk-reviewer`「风险审查员」独立检查组合敞口、压力情景和否决条件。三者主动优先调用项目知识库、新闻公告和基本面工具。研究与风控保留只读行情/K线，不挂下单/撤单连接器；公共 host 工具仍可见，角色分工不是独立安全沙箱。

base 安装器按实际启用的市场生成三种角色，支持只安装部分市场。未修改的托管旧市场预设移到同级 `.legacy-backup` 备份 root；无管理戳、修改过或含额外文件的目录原样保留，不删除用户预设数据。
