...
双语首页在原有产品叙事前增加描述性 H1、语言切换、产品摘要、章节导航与功能概览；FAQ 回答自动交易、连接器、API 密钥及商用许可问题。关键词随实际用途自然出现，不把 dry-run 等同于券商模拟盘，不把非商业许可描述为无限制开源。原有安装命令与图片路径保持不变。
...
- **堆砌关键词或嵌入 SEO meta 标签**：影响阅读，且 README 无法控制 GitHub 页面的 head；采用可见摘要、功能描述与 FAQ，不承诺搜索排名或富结果。
