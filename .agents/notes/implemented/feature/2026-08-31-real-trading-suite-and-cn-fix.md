...
- **Eastmoney (`connector-eastmoney`)**:
  - `getTicker()`: 上游 `f43` 价格字段为分（整数），增加除以 100 转换；处理停牌/无数据 `"-"` 占位符；
  - 增加茅台 ticker 价格与 5m kline close 价格 ±5% 交叉校验防回归单测。
- **AkShare (`connector-akshare`)**:
  - `getSectorFundFlow()`: 上游 `f3` 字段由整数百分比（如 `-120`）除以 100 转换为 `-1.20`；
  - `getTicker()`: `f43` 价格除以 100；
...
3. **Tiger (`connector-tiger`)**:
   - 基于 Node.js `node:crypto` 实现 TigerOpen 标准 RSA-SHA256 签名算法（`generateTigerSignature`）；
...
4. **Longbridge (`connector-longbridge`)**:
   - 基于 LongPort 官方 OpenAPI 规范实现标准签名与请求头：
     - 头部：`authorization`（Bearer token）、`x-api-key`（App Key）、`x-timestamp`（毫秒时间戳）；
