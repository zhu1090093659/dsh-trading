...
- **跨源去重**（两 kit 各自实现）：归一化标题（去 `公司:` 前缀 → hk 侧繁转
  简映射表 → 去标点空白）后判同一条披露；**同源永不去重**；先到先得保留主源
  （东财，秒级时间精度）。判重口径 2026-09-03 评审后收紧为：±24h 内**全文等值，
  或共同前缀 ≥6 字且共同后缀 ≥2 字且互不构成严格前缀**（初版仅共同前缀 ≥6 字，
  把裸类别短标题、`…报告` vs `…报告摘要`、`截至二零二六年…` 泛化日期头等同日
  不同文件误判为重复——「宁漏勿误」论断不成立，见 2026-09-03 整改 note）。
...
- **繁简标题差异**：HKEX 繁体 vs 东财转简体，且东财会缩写类别描述
  ...
  股份購回`），全文等值不可靠 → 前缀+后缀兜底。初版「共同前缀 ≥6 字」口径
  的失败方向**包含误删**（原始「宁漏勿误」论断错误）：腾讯同日连发
  `翌日披露報表`（恰 6 字）与 `翌日披露報表 - 已發行股份變動及股份購回` 即互删。
  评审整改后口径见 Decision 节。
...
- 两 fetcher 都带 10s AbortSignal + fail-soft（throw → unavailable，不拖垮
  聚合）；memo 仅缓存成功值且导出 `reset*Memo` 供单测隔离。评审整改后追加：
...
- 真实网络验证：002714.SZ 合并 6 条（巨潮 7 条原始 → 跨源重复全部去重）；
  ...
  巨潮合法空不再误报 unavailable。（口径收紧后部分跨源对保留两份，见整改 note）
- 媒体快讯第二源（新浪/财联社，spike 判 C/D 级）维持押后，另案处理。
- 巨潮源不覆盖北交所（43/83/87/92 开头）——走 szse 恒返空且与真实无公告
  不可区分，评审 M4 整改改为显式 skip；后续如需 BJ 覆盖须先补 spike 证据。
