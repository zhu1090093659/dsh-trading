...
4. **「发给 Agent」按钮**（QuoteStage 工具栏右侧，与指标选择器同组 `css.toolbarActions`）：
   ...
   - 投递编排（新 `send-to-agent.ts`，纯编排零 SDK runtime import）：`sessions.list.current` 缺席时经 uiWorkspace `startSession()` 建会话并短轮询（100ms×30）等落地 → `sessions.binding(id).session` → `beginSubmission` 本地 echo（对话列乐观气泡，图走 `previewUrl`）→ `prompt(content, 'queue', undefined, requestId)`（追加一回合不打断进行中回合；image part 只吃裸 base64，`stripDataUrlPrefix` 剥前缀）；prompt 被拒 → `abandon()` 撤 echo 后抛错；
   - 按钮态机 idle/sending/sent/error（data-state 驱动配色，2-2.6s 自动复位）；`sendToAgent` prop 可选，独立渲染/单测不注入时按钮不渲染。注入链：client `apply()` 组装（复用 `sessions` + `startNewSession` 惰性解析纪律）→ QuotePane → MiddleStage → QuoteStage。
...
- **按钮走 DOM 注入 composer（querySelector 填 textarea）**：官方 composer 是自定义元素、历史实证不吃合成输入（blank-hero composer 教训），且绕过 echo/queue 契约；`session.prompt` 是 SDK 为 feature 包显式加宽的行为面。
...
- 「发给 Agent」产出的消息是真用户消息（进 durable log、可 queue/审批链）；截图为客户端临时 image part，随会话持久化策略走宿主 attachment 通道。
- 投递依赖宿主 `sessions`/`uiWorkspace` 服务面（alpha.2 实测契约）；宿主升级若改 `beginSubmission/prompt` 签名，故障面收敛在 `send-to-agent.ts` 单文件。
- **验证记录**：pnpm build / test 全绿（654，新增 8+8 内）；trading-web profile 副本 client-ui-trading 经硬链接原地直达（inode 一致），watchlist 副本分叉需删副本重挂。实机 UI 走查见交付记录。
